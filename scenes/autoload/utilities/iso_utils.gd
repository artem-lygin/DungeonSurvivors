...
## Returns changed direction alighned with isometric radio
...
## Returns changed length Vector2 according to isometric ratio
## If "force_normailze" = true, input Vector2 will be normalised
static func to_isometric_length(vector: Vector2, force_normailze: bool = false) -> Vector2:
	if ISOMETRIC_MODE:
		if force_normailze:
